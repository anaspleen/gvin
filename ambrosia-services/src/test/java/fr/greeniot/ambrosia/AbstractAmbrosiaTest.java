...
import fr.greeniot.ambrosia.service.configuration.ConfigurationConstant;
import fr.greeniot.ambrosia.service.configuration.ConfigurationServiceInterface;
...
public abstract class AbstractAmbrosiaTest {
	...
	private ConfigurationServiceInterface m_ConfigurationService;

	/**
	 * Cette méthode est appelée avant chaque test.<br/>
	 * Nettoie la base. Peut être surchargée pour insérer des données en base
	 * avant chaque test.
	 *
	 * @throws Exception
	 *             en cas de souci.
	 ...
	 * Utile essentiellement pour relâcher des ressources allouées avec la
	 * méthode setUp().
	 *
	 * @throws Exception
	 *             en cas de souci.
	 ...
	 * @return le client
	 */
	protected MongoClient getClient() throws Exception {
		MongoClient client = new MongoClient(
				new MongoClientURI(m_ConfigurationService.getParameterString(ConfigurationConstant.DATABASE_URL)));
		return client;
	}
	 ...
	 * @return la base
	 */
	protected MongoDatabase getDatabase() throws Exception {
		MongoDatabase database = getClient()
				.getDatabase(m_ConfigurationService.getParameterString(ConfigurationConstant.DATABASE_NAME));
		return database;
	}
